...
/** Get max counter days among all ingredients of a meal — uses STRICT name matching only */
function getMaxIngredientCounter(meal: Meal, foodItems: FoodItem[]): number | null {
  if (!meal.ingredients?.trim()) return null;
  const groups = parseIngredientGroups(meal.ingredients);
  let maxDays: number | null = null;
  let maxIngName: string | null = null;
...
/** Get the ingredient name with max counter */
function getMaxIngredientCounterName(meal: Meal, foodItems: FoodItem[]): string | null {
  if (!meal.ingredients?.trim()) return null;
  const groups = parseIngredientGroups(meal.ingredients);
  let maxDays: number | null = null;
  let maxIngName: string | null = null;
